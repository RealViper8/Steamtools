...
const MANIFESTS_URL: &str =
    "https://raw.githubusercontent.com/SteamAutoCracks/ManifestHub/refs/heads";
...
fn install(path: &String, appid: i32) -> io::Result<()> {
    ...
    let resp = reqwest::blocking::get(sb.as_str())
        .map_err(|_| Error::new(ErrorKind::ConnectionAborted, "Invalid url"))?;
    ...
    write!(
        sb,
        "{}{MAIN_SEPARATOR}config{MAIN_SEPARATOR}stplug-in{MAIN_SEPARATOR}{}.lua",
        path, appid
    )
    .unwrap();
            ...
            .show(ui, |ui| {
                ui.vertical_centered(|ui| {
                    ui.horizontal(|ui| {
                        ...
                        if response.changed()
                            && !app.install.appid.chars().all(|c| c.is_ascii_digit())
                        {
                    ...
                    if ui.button("Download").clicked() && !app.install.appid.is_empty() {
                        if let Err(error) = (app.install.handler.0)(
                            &app.st.path,
                            app.install.appid.parse::<i32>().unwrap(),
                        ) {
                            match error.kind() {
                                ...
                                ErrorKind::ConnectionRefused => {
                                    rfd::MessageDialog::new()
                                        ...
                                        .set_description(
                                            "Couldnt find app in database / invalid appid",
                                        )
                                        .set_buttons(rfd::MessageButtons::Ok)
                                        .show();
                                }
