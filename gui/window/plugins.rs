                            ...
                            Ok(d) => {
                                app.plugins.list.push(Plugin {
                                    ...
                                    name: Arc::new(Mutex::new(
                                        d.path().file_stem().unwrap().to_string_lossy().to_string(),
                                    )),
                                    name_buffer: d
                                        .path()
                                        .file_stem()
                            ...
                            for (i, plugin) in app.plugins.list.iter().enumerate() {
                                ui.group(|ui| {
                                    ui.vertical_centered(|ui| {
                                        ui.add(
                                            Label::new(
                                                RichText::new(plugin.name.lock().unwrap().as_str())
                        ...
                        ui.horizontal(|ui| {
                                ...
                                || ui.input(|i| i.modifiers.ctrl && i.key_pressed(egui::Key::S))
                            {
                                fs::rename(
                                    format!(
                                        "./plugins/{}.lua",
                                        &app.plugins.get_selected().unwrap().name.lock().unwrap()
                                    ),
                                    format!(
                                        "./plugins/{}.lua",
                                        &app.plugins.get_selected().unwrap().name_buffer
                                    ),
                                )
                                .ok();
                                fs::write(
                                    ...
                                    app.plugins.get_selected().unwrap().code.as_bytes(),
                                )
                                .unwrap();
                                *app.plugins
                                    .list
                                    .get_mut(app.plugins.selected_plugin.unwrap())
                                    .unwrap()
                                    .name
                                    .lock()
                                    .unwrap() =
                                    app.plugins.get_selected().unwrap().name_buffer.clone();
                            }

                            ui.text_edit_singleline(
                                &mut app.plugins.get_selected().unwrap().name_buffer,
                            );
                        });
                    });
